...
public class RoomService {
    ...
    public void create(String email, Long hotelId, RoomDTO.Request request, MultipartFile file) {
        Room room = create(email, hotelId, request);
        uploadThumbnail(email, hotelId, room.getId(), file);
    }

    public void uploadThumbnail(String email, Long hotelId, Long roomId, MultipartFile file) {
        hotelService.validMasterMemberOrThrow(email);
        hotelRepository.findById(hotelId)
          .orElseThrow(() -> new ApiException(ApiErrorCode.NOT_FOUND_HOTEL));
        roomRepository.findById(roomId)
          .map(room -> {
              String filename = String.format("hotel-%s-room-%s", hotelId, roomId);
              try {
                  String upload = imageService.upload(file, filename);
                  room.updateThumbnail(upload);
              } catch (IOException e) {
